...
type DemoChatMessage = { id: string; author: "agent" | "visitor"; text: string };
...
function AgentDemo({ onBack }: { onBack: () => void }) {
  ...
  const send = async (rawText: string) => {
    ...
    try {
      const response = await api<{ messages: { id: string; text: string }[] }>("/api/public/agent/messages", { method: "POST", body: JSON.stringify({ sessionId, text }) });
