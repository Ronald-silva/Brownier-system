...
export function AdminOperations({ onExit }: { onExit: () => void }) {
  ...
  {tab === "products" && <section className="admin-content"><p className="eyebrow">CARDÁPIO DE HOJE</p><h1>Disponibilidade em um toque.</h1><p className="subtle">As mudanças aparecem imediatamente para o cliente.</p><h2 className="sr-only">Lista de sabores</h2><div className="admin-list">{store.products.map(p => <article className="op-product" key={p.id}><label className="photo-upload"><img src={p.imageUrl || "/images/brownie-hero-demo.png"} alt={`Foto de ${p.name}`} /><input type="file" accept="image/*" onChange={e => uploadPhoto(p, e.target.files?.[0])} /></label><div><h3>{p.name}</h3><p>{formatCurrency(p.basePrice)}{p.promotionalPrice && ` · ${formatCurrency(p.promotionalPrice)} a partir de ${p.minimumPromotionalQuantity}`}</p><div className="product-flags">{p.isDay && <b>Brownie do Dia</b>}{p.isFeatured && <b>Destaque</b>}</div></div><button className={p.isAvailable?"switch on":"switch"} aria-label={`Alterar disponibilidade de ${p.name}`} onClick={() => toggleAvailability(p)}><i /></button><button className="day-toggle" onClick={() => toggleDay(p)}>☆ Dia</button><details><summary>Editar</summary><ProductEditor product={p} onCancel={() => {}} onSave={async fields => { await product(p, fields, "Sabor atualizado."); }} onDelete={() => deleteProduct(p)} /></details></article>)}</div></section>}
